...
async function probeEndpoints(log){
  ...
  await Promise.all(ENDPOINTS.map(async url => {
    ...
    try{
      ...
      noteFail(url); noteFail(url); noteFail(url);
      say(`osm: ${host(url)} is answering ${r.status}, benched for 15 minutes`);
    }catch(e){
      noteFail(url); noteFail(url); noteFail(url);
      say(`osm: ${host(url)} is not reachable (${e.name === 'AbortError' ? 'timed out' : e.message}), benched for 15 minutes`);
    }finally{ clearTimeout(timer); }
  }));
  const live = ENDPOINTS.filter(u=>!benched(u));
  say(`osm: ${live.length} of ${ENDPOINTS.length} mirrors answering${live.length ? ' — ' + live.map(host).join(', ') : ''}`);
  return live.length;
}
