...
/**
 * One request per country, and — this is the part that matters — one area
 * lookup per sport rather than one per sport per contact tag.
 *
 * Insisting on ["website"] inside the query looked like a free narrowing.
 * It is not: each extra tag filter is another statement, and every
 * statement repeats the area lookup, which is the expensive half. Five
 * sports times four contact keys was twenty-four scans of the country per
 * import. Malta — a country with six racket venues in the whole database —
 * went from seconds to a 197-second timeout.
 *
 * So the query asks only for named places that play one of the sports, and
 * the contact filtering happens here in JavaScript, where it is free. That
 * also brings back the leads: clubs with a name and no contact tagged, worth
 * keeping so they are not rediscovered on every run.
 *
 * club=sport is excluded on purpose. It would pull in every judo, football
 * and sailing club in the country — on the Malta test that was most of the
 * response.
 */
function buildQuery(cc){
  const branches = SPORT_VALUES.map(v => `  nwr(area.c)["sport"="${v}"]["name"];`)
    .concat('  nwr(area.c)["club"~"^(tennis|padel|squash|racquet)$"]["name"];');
...
async function overpass(cc, deadline, epOffset){
  if(deadline && Date.now() > deadline) return {elements: null, error: 'out of time'};

  const r = await runQuery(buildQuery(cc), epOffset, deadline);
  if(!r.elements) return {elements: null, error: r.error};

  // Deduplicated because a place tagged sport=tennis and club=tennis matches
  // two of the branches in the union.
  const seen = new Map();
  for(const el of r.elements) seen.set(el.type+'/'+el.id, el);
  return {elements: Array.from(seen.values()), endpoint: r.endpoint};
}
