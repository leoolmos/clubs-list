 ...
 *   maps.mail.ru (VK)      "Rate limit: 0"   no concurrency limit
 *
 ...
 * mirrors carry different snapshots, and on one Malta test they returned 6,
 * 4 and 2 places — so it is worth still using, just not worth queueing on.
 *
 ...
 *
 * Three mirrors was one outage away from nothing, and on 2026-08-31 that is
 * exactly what happened: kumi and private.coffee answered 502, and
 * overpass-api.de stopped answering this address at all — the TCP connect
 * times out on both of its IPs while the rest of the internet, openstreetmap
 * .org included, is fine. That is a firewall, not a busy server. So the VK
 * mirror is in the list as a fourth, independently operated fallback, and
 * mirrors that stop answering are benched (see `benched` below) instead of
 * being retried by every country in the round until the round is over.
 *
 * Only whole-planet mirrors belong here. A regional extract — overpass.osm.ch
 * is the obvious one — answers a Malta query with HTTP 200 and zero elements,
 * which reads as "this country has no clubs" and would file it as done. */
const ENDPOINTS = [
...
/* A contact that goes somewhere. The placeholder address that used to sit
 * here is worse than no contact at all: it is the mark of a script nobody
 * is answering for, and it is what an operator blocks first. */
const UA = 'RacketClubResearch/1.0 (club directory research; +https://github.com/leoolmos/clubs-list)';

/* Benching a mirror that has stopped answering.
 *
 * Without this, a dead mirror costs every request in the round its full
 * timeout before the next one is tried — six countries times four statements
 * times two attempts against two dead mirrors is the whole eight-minute slice
 * spent on servers that are not there. Three consecutive failures and the
 * mirror sits out for fifteen minutes.
 *
 * 429 is not a failure of this kind. It is the rate limiter working, and the
 * mirror is healthy; benching it would throw away a good server. */
const BENCH_AFTER  = 3;
const BENCH_MS     = 15*60*1000;
/* Long enough that a slow mirror is not mistaken for a dead one: mail.ru's
 * status page has taken eleven seconds while the mirror itself was healthy. */
const PROBE_TIMEOUT_MS = 30000;
const health = new Map();          // url -> {fails, until}

function benched(url){
...
function noteFail(url){
  ...
  h.fails++;
  if(h.fails >= BENCH_AFTER) h.until = Date.now() + BENCH_MS;
  health.set(url, h);
}
function noteOk(url){ health.delete(url); }
/* Straight to the bench, for a mirror that has answered with something
 * unambiguous — an HTTP error, or a connection that goes nowhere. */
function bench(url){
  health.set(url, {fails: BENCH_AFTER, until: Date.now() + BENCH_MS});
}

/* The mirrors to try, best first. Benched ones are dropped — unless they all
 * are, in which case the one closest to coming back is tried anyway, so a
 * total outage recovers by itself instead of needing a restart. */
function endpointOrder(epOffset){
  const order = ENDPOINTS.map((_, i) => ENDPOINTS[(i + (epOffset||0)) % ENDPOINTS.length]);
  const live  = order.filter(u => !benched(u));
  if(live.length) return live;
  return [order.slice().sort((a,b)=>(health.get(a).until||0)-(health.get(b).until||0))[0]];
}

/* One cheap request per mirror, before the countries start. /api/status is a
 * few lines of text and no query load, so this costs nothing and tells the
 * round which mirrors are worth talking to — instead of every country
 * finding out the hard way, one 120-second timeout at a time. */
async function probeEndpoints(log){
  const say = log || (()=>{});

  /* Slow is not dead, and the difference decides whether the round happens.
   * maps.mail.ru takes ten to thirty seconds to answer /api/status while
   * serving country queries perfectly well — a 599-place Argentina came
   * through it minutes either side of a status call that never returned. A
   * probe that benched it on that would throw away the only mirror that was
   * working and skip the round for fifteen minutes.
   *
   * So only a definite failure benches a mirror: a refused or unroutable
   * connection, or an HTTP error. Our own timeout means we gave up on it,
   * not that it is gone, and the mirror is left in with the benefit of the
   * doubt — a real country query against it costs a request and settles the
   * question properly. */
  await Promise.all(ENDPOINTS.map(async url => {
    const status = url.replace(/\/interpreter$/, '/status');
    const ctl = new AbortController();
    const timer = setTimeout(()=>ctl.abort(), PROBE_TIMEOUT_MS);
    try{
      const r = await fetch(status, {headers:{'User-Agent':UA}, signal: ctl.signal});
      if(r.ok){ noteOk(url); return; }
      bench(url);
      say(`osm: ${host(url)} is answering ${r.status}, benched for 15 minutes`);
    }catch(e){
      if(e.name === 'AbortError'){
        say(`osm: ${host(url)} did not answer its status page in ${Math.round(PROBE_TIMEOUT_MS/1000)}s — slow, not benched`);
        return;
      }
      bench(url);
      const why = (e.cause && e.cause.message) || e.message;
      say(`osm: ${host(url)} is not reachable (${why}), benched for 15 minutes`);
    }finally{ clearTimeout(timer); }
  }));

  const live = ENDPOINTS.filter(u=>!benched(u));
  say(`osm: ${live.length} of ${ENDPOINTS.length} mirrors worth trying${live.length ? ' — ' + live.map(host).join(', ') : ''}`);
  return live.length;
}
...
async function runQuery(query, epOffset, deadline){
  ...
  // limiter and the parallelism would buy nothing. Mirrors that have stopped
  // answering are left out entirely — see `benched`.
  const order = endpointOrder(epOffset);
    ...
    for(let attempt=0; attempt<2; attempt++){
      ...
      try{
        ...
        if(r.status === 429 || r.status === 406 || r.status >= 500){
          ...
          // 429 is the rate limiter and the mirror is healthy; 5xx and 406
          // are the mirror or its gateway failing, and a mirror that does
          // that three times running is worth skipping for a while.
          if(r.status !== 429) noteFail(url);
          continue;
        }
        // A 400 really is this query being wrong, and asking again would
        // only be rude to a donated server.
        if(!r.ok){ lastErr = 'HTTP '+r.status; break; }   // our query, not their server
        ...
        // A refused or timed-out connection is the mirror being unreachable,
        // which is the case benching exists for.
...
async function overpass(cc, deadline, epOffset, opts){
  ...
  if(!areas || !areas.length){
    /* The subdivision lookup gets a budget of its own, not the whole slice.
     *
     * It is a light query — relation ids and names, nothing else — but it
     * queues behind everything else on a busy mirror, and with the round's
     * deadline it will wait for all of it. Measured: Brazil spent seven of
     * an eight-minute slice waiting for the list of its own states, failed,
     * saved nothing, and left the fourth country in the batch unstarted.
     * Failing fast hands those minutes to a country that can use them. */
    const subDeadline = deadline ? Math.min(deadline, Date.now() + SUBAREA_BUDGET_MS) : null;
    const sub = await runQuery(buildSubAreaQuery(cc), epOffset, subDeadline);
  ...
  /* Hand the subdivision list back even when the round achieved nothing.
   *
   * A country's states do not change, so looking them up is a once-ever cost
   * — but it was being paid again every round, because the list was only
   * written to the state file when the country came back without an error.
   * Brazil would spend its slice discovering that it has 27 states, run out
   * of round before reading any of them, save nothing, and start the round
   * after by discovering that it has 27 states. Returning them here means
   * the next round opens straight into a state-sized query. */
  if(!seen.size && failed.length && !done.length)
    return {elements: null, error: failed[0], areas, doneAreas: done};
  return {
...
async function importCountry(cc, db, log, deadline, opts){
  ...
  const {elements, error, endpoint, split, areas, doneAreas} = await overpass(cc, deadline, opts.epOffset, {
    split: !!opts.split, areas: opts.areas, doneAreas: opts.doneAreas
  });
  // A failed round that nonetheless learned the country's subdivisions hands
  // them up, so the next one starts from them rather than looking again.
  if(!elements) return {cc, error: error || 'no response', areas, doneAreas};
  if(split){
...
async function importMany(codes, db, log, deadline, parallel){
  ...
  // Which mirrors are up, asked once and cheaply, before any country starts.
  // With none of them up there is nothing to do but say so and give the round
  // back to the crawler — which needs no Overpass at all — rather than spend
  // it timing out.
  if(!(await probeEndpoints(say))){
    say('osm: no Overpass mirror is answering, so the round goes to the crawler instead');
    return results;
  }
  ...
  async function worker(slot){
    while(true){
      ...
      /* Do not start a country there is no time to get anywhere with.
       *
       * The time is checked before the country is taken off the list, so an
       * unstarted country stays unstarted rather than being consumed and
       * skipped. A big country's first act is a subdivision lookup — one
       * more Overpass query, against the country as a whole — and starting
       * that with twenty seconds left spends the twenty seconds, saves
       * nothing, and files the country as failed. Three of those in a row
       * used to convince the round that Overpass was down. */
      if(deadline && Date.now() > deadline - MIN_COUNTRY_MS) return;
      ...
      /* Running out of the round is this collector's budget expiring, not
       * Overpass saying no, and the two were being recorded as the same
       * thing. A country that timed out was filed with `error: out of time`,
       * which the status page reads as a refusal — so the banner said
       * "OpenStreetMap is refusing queries" through rounds where every
       * mirror answered every request it was given.
       *
       * It also has to keep the `partial:` marker it already had. Overwriting
       * that with a timeout turns a country that is 7 of 80 subdivisions in
       * into one that looks refused, and the page loses the only sign that
       * the biggest countries are moving at all. */
      const ranOutOfRound = s.error && /out of time/i.test(s.error);

      if(s.error){
        // Keep whatever subdivision progress the country already had, so a
        // failed round does not throw away an hour of reading.
        const prev = state[cc] || {};
        const keepPartial = ranOutOfRound && /^partial:/.test(String(prev.error || ''));
        state[cc] = Object.assign({}, prev, {
          at: new Date().toISOString(),
          error: keepPartial ? prev.error : s.error
        });
        // Subdivisions found on the way to failing are still worth keeping.
        if(s.areas && s.areas.length){
          state[cc].split = true;
          state[cc].areas = s.areas;
          state[cc].doneAreas = s.doneAreas || prev.doneAreas || [];
        }
      } else {
      ...
      results.push(s);
      if(s.error){
        /* The circuit breaker exists to stop a round hammering mirrors that
         * are down. Time running out says nothing about the mirrors, so it
         * must not trip it — and it did: with twelve countries sharing a
         * five-minute slice, three would time out together and the round
         * would announce that Overpass was not answering today while every
         * mirror was answering fine. */
        if(ranOutOfRound){
          say(`osm ${cc}: ran out of the round — picked up where it left off next time`);
          continue;
        }
...
/* The least time worth giving a country. A subdivision lookup plus one
 * statement against a state-sized area is about this, and below it the round
 * only manages to fail. */
const MIN_COUNTRY_MS = 90*1000;

/* How long the one-off lookup of a country's states may take before the
 * country gives its turn back. */
const SUBAREA_BUDGET_MS = 150*1000;
...
module.exports = { importCountry, importMany, nextCountry, nextCountries, markDone,
                   buildQuery, toRecord, keyFor, probeEndpoints, ENDPOINTS };
