...
async function overpass(cc, deadline, epOffset){
  ...
  for(let i=0;i<STATEMENTS.length;i++){
    ...
    if(!r.elements){
      failed.push(r.error);
      // If the first statement cannot be served by any mirror, the country
      // is not available right now and the other three will fail the same
      // way. Trying them anyway made a dead country take four times as long
      // to give up on, which in turn delayed the circuit breaker that hands
      // the round back to the crawler.
      if(i === 0) return {elements: null, error: r.error};
      continue;
    }
...
function nextCountries(n){
  ...
  const failedRecently = cc => {
    const s = state[cc];
    return s && s.error && s.at && (now - new Date(s.at).getTime()) < RETRY_AFTER_MS;
  };
