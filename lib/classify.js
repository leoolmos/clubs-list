...
/* Pickleball is the fourth sport in the brief. It spells itself the same way
 * everywhere the collector looks, so one pattern covers all three languages;
 * the hyphen and the space are what club names actually vary on. */
const RE_PICKLEBALL = /pickle[\s_-]?ball/i;

/** The only four sports this database is about, in the order the page shows. */
const SPORTS = ['Tennis','Padel','Squash','Pickleball'];
...
/**
 * Racket sports that are not one of the four, written the way they collide.
 *
 * Every one of these carries a word the tests above look for, so without
 * this a hall that plays none of the four is published as a club that does.
 * Measured on the first Overture scan: "Tenis de Mesa Lidia Calderón" and
 * "Oficial Tenis de Mesa" came back as Colombian tennis clubs, and "Frontón
 * Albarrán" as a Mexican one. The hole was never Overture's — "Tênis de
 * Mesa" reads as tennis to every source here, so a Brazilian table-tennis
 * hall found by the prospector or listed on a directory page would have been
 * published just the same.
 *
 * These are removed from the text before the four are looked for, rather
 * than used to veto the record, because a club with both is still a club:
 * "Tênis e Tênis de Mesa" is a common pairing in Brazil and the tennis half
 * is real. Only a place whose *sole* racket sport is one of these ends up
 * with nothing and is dropped.
 *
 * Beach tennis is in the list on purpose. It is its own sport with its own
 * courts and its own federation, and in Brazil there are whole venues that
 * play nothing else — a strict reading of the four, which is what was
 * asked for.
 *
 * "racquetball" and "racquet ball" only: a plain "Racquet Club" is very
 * often exactly the kind of members' tennis and squash club this is for.
 */
const RE_OTHER_SPORT = new RegExp([
  't[eéê]nn?is[\\s_-]+de[\\s_-]+mesa', 'table[\\s_-]+tennis', 'ping[\\s_-]?pong',
  'mesa[\\s_-]+de[\\s_-]+t[eéê]nn?is',
  'beach[\\s_-]+t[eéê]nn?is', 't[eéê]nn?is[\\s_-]+de[\\s_-]+praia', 't[eéê]nn?is[\\s_-]+playa',
  'racquet[\\s_-]?ball', 'raqu?et[\\s_-]?bol',
  'badmin?ton', 'front[oó]n', 'frontenis', 'pelota[\\s_-]+vasca', 'pickle[\\s_-]?ball[\\s_-]+paddle'
].join('|'), 'gi');

/**
 * Does this text name a racket sport that is not one of the four?
 *
 * Exported because a category and a name can disagree, and the caller needs
 * to know which way. Written as a replace rather than a test: RE_OTHER_SPORT
 * is global, and `test` on a global regexp carries a cursor between calls, so
 * it answers true, then false, then true for the very same string.
 */
function namesOtherSport(text){
  const t = String(text||'');
  return t.replace(RE_OTHER_SPORT, ' ') !== t;
}

function detectSports(text){
  const t = String(text||'');
  /* RE_OTHER_SPORT carries the global flag so every mention goes, not just
   * the first. It must only ever be used with replace: `test` on a global
   * regexp keeps a cursor between calls and answers differently each time. */
  const only = t.replace(RE_OTHER_SPORT, ' ');
  const s = [];
  const padel = RE_PADEL.test(only) || RE_PADEL_TENNIS.test(only);
  const tennisOnlyFromPadel = RE_PADEL_TENNIS.test(only) && !RE_TENNIS.test(only.replace(RE_PADEL_TENNIS,' '));
  if(RE_TENNIS.test(only) && !tennisOnlyFromPadel) s.push('Tennis');
  if(padel) s.push('Padel');
  if(RE_SQUASH.test(only)) s.push('Squash');
  if(RE_PICKLEBALL.test(only)) s.push('Pickleball');
  return s;
}

/* Same idea, from OSM's sport=* tag, which is a semicolon list, and from
 * Overture's category, which is one snake_case word. OSM's `table_tennis`
 * never matched here — an underscore is a word character, so `\btennis\b`
 * does not see inside it — but a category read with the underscores opened
 * out to spaces would, so it is stripped explicitly rather than left to a
 * word boundary that only holds in one of the two spellings. */
function sportsFromTag(tag){
  const t = String(tag||'').toLowerCase().replace(RE_OTHER_SPORT, ' ');
  const s = [];
  ...
  if(/pickle[\s_-]?ball/.test(t)) s.push('Pickleball');
...
const RE_CLUBWORD = new RegExp([
  ...
  'p[aá]del','padel','squash','pickle[\\s-]?ball','sports? club','sporting','\\bacademy\\b','academia',
...
const JUNK = new RegExp([
  ...
  // The crawl reads privacy policies and legal pages now, because they print
  // an address on sites that have no contact page — and they also print the
  // regulator's. A London tennis club came back as international.team@
  // ico.org.uk, which is the Information Commissioner, not the club.
  'ico\\.org\\.uk','aepd\\.es','cnpd\\.pt','anpd\\.','edpb\\.europa','europa\\.eu',
  'datenschutz','garanteprivacy','cnil\\.fr','oaic\\.gov','priv\\.gc\\.ca',
  'dataprotection\\.ie','\\.europa\\.eu$','wix\\.com$','shopify\\.com$','google\\.com$',
  'apple\\.com$','microsoft\\.com$','facebook\\.com$','meta\\.com$'
].join('|'),'i');

/* Addresses that exist but are not a way to reach the club */
const RE_NOREPLY = /^(no-?reply|donot-?reply|postmaster|hostmaster|abuse|privacy|dmarc|dkim|spam|webmaster|wordpress|admin@localhost|sales@wix|dpo@|data-?protection|gdpr@|rgpd@|lgpd@|datenschutz)/i;
...
function extractEmails(html, host){
  ...
  for(const m of src.match(/mailto:([^"'?\s>]+)/gi)||[]){
    let e = m.replace(/mailto:/i,'');
    try{ e = decodeURIComponent(e); }catch(_){}
    found.add(e.toLowerCase());
  }

  // The jQuery-era protection plugins keep the two halves in attributes:
  // <a data-user="info" data-domain="club.com"> and the name/domain pair.
  for(const m of src.matchAll(/data-(?:user|name|mail-user)\s*=\s*["']([^"'@\s]+)["'][^>]{0,200}?data-(?:domain|mail-domain)\s*=\s*["']([^"'@\s]+)["']/gi)){
    found.add((m[1] + '@' + m[2]).toLowerCase());
  }
  for(const m of src.matchAll(/data-(?:domain|mail-domain)\s*=\s*["']([^"'@\s]+)["'][^>]{0,200}?data-(?:user|name|mail-user)\s*=\s*["']([^"'@\s]+)["']/gi)){
    found.add((m[2] + '@' + m[1]).toLowerCase());
  }

  // "info [at] club [dot] com" and its many spellings — and the ways a page
  // hides the @ from a regex: entities (decimal and hex), the JavaScript
  // escape, the URL escape, and 'info' + '@' + 'club.com' joined in script.
  const deob = src
    .replace(/&#0?64;|&#x0*40;|&commat;|\\u0040|%40/gi,'@')
    .replace(/&#0?46;|&#x0*2e;|&period;/gi,'.')
    .replace(/['"`]\s*\+\s*['"`]/g,'')
    .replace(/\s*[\[({]\s*(at|arroba|arobase|chiocciola)\s*[\])}]\s*/gi,'@')
    .replace(/\s*[\[({]\s*(dot|punto|ponto)\s*[\])}]\s*/gi,'.')
    .replace(/\s+(at|arroba)\s+/gi,'@').replace(/\s+(dot|punto|ponto)\s+/gi,'.');
...
module.exports = {
  SPORTS,
  detectSports, sportsFromTag, namesOtherSport, privateClub, extractEmails, extractContactName,
