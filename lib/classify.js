 ...
 * Sports
 * ------------------------------------------------------------------ */
/* Prefix matches, not whole words, because half of Europe writes the sport
 * and the club as one word: Tennisverein, tennisklubb, tenisový klub,
 * squashhalle. Requiring \b after the sport dropped every one of them. */
const RE_TENNIS = /\bt[eé]nn?is|lawn tennis/i;
const RE_PADEL  = /p[aá]del/i;
const RE_SQUASH = /\bsquash|\bskvo[šs]/i;
...
const RE_PUBLIC = new RegExp([
  ...
  'f[eé]d[eé]ration','federazione','federatie','\\bverband\\b','f[öo]rbund','\\bligue\\b',
  // local government in the expansion languages
  '\\bmairie\\b','\\bville de\\b','\\bcommune de\\b','\\bcomune di\\b',
  '\\bgemeente\\b','\\bgemeinde\\b','st[aä]dtisch','\\bkommun(e|al|en)?\\b',
  // state and forces
...
const RE_CLUBWORD = new RegExp([
  ...
  'racquet','racket','lawn tennis','tennis club','tenis club','t[eé]nis club',
  'p[aá]del','padel','squash','sports? club','sporting','\\bacademy\\b','academia',
  'centre de tennis','centro de tenis','centro de t[eé]nis','\\bcourts?\\b',
  ...
  // The expansion languages write "club" onto the sport as one word —
  // Tennisclub, tennisklubb, tenisový klub, tennisvereniging — so these are
  // suffix and stem matches rather than whole words.
  'club\\b','klub(b|i|s|as)?\\b','verein\\b','vereniging\\b','seura\\b','f[eé]lag\\b',
  'cent(er|re)\\b','\\btc\\b','\\bltc\\b','\\btk\\b',
  'associazione sportiva','societ[aà] sportiva','polisportiva'
].join('|'),'i');
...
const STOPWORDS = new Set([
  ...
  // the one-word forms the expansion languages use — as distinctive tokens
  // they would match any club's domain, which is the sponsor bug all over
  'tennisclub','tennisverein','tennisvereniging','tennisklubb','tennisklub',
  'tennisseura','padelclub','padelklubb','padelklub','squashclub','squashklubb',
  'sportverein','turnverein','vereniging','verein','klubb','klub','klubs','klubas',
  'klubi','seura','circolo','cercle','und','der','die','das','van','den','het',
  'les','des','dei','degli'
]);
...
const FREEMAIL = new Set([
  ...
  // the expansion countries' big providers
  'wanadoo.fr','orange.fr','laposte.net','sfr.fr','hotmail.fr','yahoo.fr',
  'gmx.de','web.de','t-online.de','freenet.de','hotmail.de','yahoo.de','posteo.de',
  'libero.it','tiscali.it','virgilio.it','alice.it','tin.it','hotmail.it','yahoo.it',
  'ziggo.nl','kpnmail.nl','planet.nl','home.nl','hetnet.nl','xs4all.nl','hotmail.nl',
  'telenet.be','skynet.be','proximus.be','bluewin.ch','sunrise.ch',
  'telia.com','online.no','mail.dk','elisanet.fi','luukku.com','simnet.is',
  'wp.pl','o2.pl','interia.pl','onet.pl','seznam.cz','email.cz','centrum.cz',
  'azet.sk','freemail.hu','citromail.hu','abv.bg'
]);
