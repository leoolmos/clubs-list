...
/* Every query one country is ever asked, in the order it is asked: the
 * country-wide set first, then one set per city, biggest city first. The
 * city is carried along so the vet can accept it as proof of place.
 *
 * Country and city in every query, both in the country's own language:
 * "clube de tênis Santos Brasil contato", "club de tenis Córdoba Argentina
 * contacto" — the country keeps Córdoba out of Spain and Santos out of the
 * search for some other Santos, and the local name is the one that ranks
 * the local pages. Term i carries contact word i: the two spellings take
 * turns across the terms, so every city is asked both ways without
 * doubling the queries.
 *
 * One function, because three things must agree on the list: the search
 * itself, the daemon's "is every query done" check that ends a pass, and
 * scripts/restore-prospect.js, which rebuilds the ledger from a published
 * status.json. A city carrying the country's own name asks the very
 * queries the country-wide pass asks, so it gets no set of its own. */
function queriesFor(cc){
  ...
  return terms.map((t, i) => ({q: `${t} ${local} ${word(i)}`, also: twin}))
...
module.exports = { prospectCountry, queriesFor, vet, nameFromTitle, nameCandidates, genericName, twinCity, termsFor, TERMS, CITIES };
