...
const TERMS_BY_CC = {
  BR: ['clube de tênis', 'clube de padel', 'clube de squash',
       'academia de tênis', 'centro de padel', 'clube de tenis']
};
const CONTACT_WORD_BY_CC = { BR: 'contato' };
...
async function vet(url, cc, timeoutMs, city){
  ...
  const name  = named.find(n => RE_CLUBWORD.test(n)) || nameFromTitle(title);
  ...
  return {
    rec: {
      ...
      // emails the crawl later finds, city by city
      city: city || '',
...
async function prospectCountry(cc, opts){
  ...
  const word     = CONTACT_WORD_BY_CC[cc] || CONTACT_WORD[lang] || CONTACT_WORD.English;
  ...
  for(const {q, city, also} of queries){
    ...
    pc.s++;
    // The twin city is credited with the search, not with the clubs: a club
    // this query turns up is recorded country-wide, and counting it in both
    // places would report it twice.
...
module.exports = { prospectCountry, vet, nameFromTitle, nameCandidates, twinCity, termsFor, TERMS, CITIES };
