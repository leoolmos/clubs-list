...
async function prospectCountry(cc, opts){
  ...
  for(const {q, city, also} of queries){
    ...
    // query is not marked done and the city is not credited with it, so it
    // is asked again next round from exactly here.
    if(results.throttled){ out.throttled = true; out.throttleWhy = results.why; break; }
    done.add(q);
    out.lastQuery = q;
    const cityKey = city || '';
