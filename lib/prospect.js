...
const TERMS = {
  ...
  French:     ['club de tennis', 'club de padel', 'club de squash'],
  German:     ['tennisclub', 'padelclub', 'squashclub'],
  Italian:    ['circolo tennis', 'club padel', 'club squash'],
  Dutch:      ['tennisvereniging', 'padelclub', 'squashclub'],
  Swedish:    ['tennisklubb', 'padelklubb', 'squashklubb'],
  Norwegian:  ['tennisklubb', 'padelklubb', 'squashklubb'],
  Danish:     ['tennisklub', 'padelklub', 'squashklub'],
  Finnish:    ['tennisseura', 'padelseura', 'squashseura'],
  Polish:     ['klub tenisowy', 'padel klub', 'squash klub'],
  Czech:      ['tenisov\u00fd klub', 'padelov\u00fd klub', 'squashov\u00fd klub'],
  Slovak:     ['tenisov\u00fd klub', 'padelov\u00fd klub', 'squashov\u00fd klub'],
  Hungarian:  ['teniszklub', 'padel klub', 'squash klub'],
  Slovenian:  ['teni\u0161ki klub', 'padel klub', 'squash klub'],
  Croatian:   ['teniski klub', 'padel klub', 'squash klub'],
  Romanian:   ['club de tenis', 'club de padel', 'club de squash'],
  Bulgarian:  ['tenis klub', 'padel klub', 'squash klub'],
  Estonian:   ['tenniseklubi', 'padeliklubi', 'squashiklubi'],
  Latvian:    ['tenisa klubs', 'padela klubs', 'skvo\u0161a klubs'],
  Lithuanian: ['teniso klubas', 'padelio klubas', 'skvo\u0161o klubas']
  // Greek and Icelandic fall through to the English terms on purpose: their
  // club sites rank for them, and the local scripts would fail the club-word
...
const CONTACT_WORD = {
  ...
  French: 'contact', German: 'kontakt', Italian: 'contatti', Dutch: 'contact',
  Swedish: 'kontakt', Norwegian: 'kontakt', Danish: 'kontakt', Finnish: 'yhteystiedot',
  Polish: 'kontakt', Czech: 'kontakt', Slovak: 'kontakt', Hungarian: 'kapcsolat',
  Slovenian: 'kontakt', Croatian: 'kontakt', Romanian: 'contact', Bulgarian: 'kontakti',
  Estonian: 'kontakt', Latvian: 'kontakti', Lithuanian: 'kontaktai'
};
 ...
 */
async function vet(url, cc, timeoutMs){
  let origin, host;
  ...
  let tied = hostCc === cc || RE_COUNTRY.test(text);
  ...
  if(!tied){
    for(const p of ['/contact', '/contacto']){
      ...
      if(page && RE_COUNTRY.test(textOf(page))){ tied = true; break; }
    }
  }
  if(!tied) return {reason:'nothing on its pages ties it to '+country};
...
async function prospectCountry(cc, opts){
  ...
  const queries = terms.map(t => `${t} ${country} ${word}`);
  ...
  for(const q of queries){
    ...
    for(const r of results){
      ...
      out.candidates++;
      const v = await vet(r.url, cc, opts.timeoutMs);
      if(v.rec){
...
module.exports = { prospectCountry, vet, nameFromTitle, TERMS };
