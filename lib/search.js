...
const GAP_MS = parseInt(process.env.SEARCH_GAP_MS || '4000', 10);
let lastQuery = 0;
...
async function searchWeb(query, timeoutMs){
  ...
  for(let attempt = 0; attempt < 3 && !html; attempt++){
    ...
    const base = attempt < 2 ? GAP_MS : RETRY_WAIT_MS;
  ...
  if(PAGES > 1 && (html.match(/uddg=/g) || []).length >= 10){
    const wait = Math.round(GAP_MS * (0.6 + Math.random() * 0.8)) - (Date.now() - lastQuery);
    if(wait > 0) await sleep(wait);
    lastQuery = Date.now();
...
module.exports = { searchWeb, findClubSite, searchCoolingDown, ENDPOINT, ENDPOINTS, NOT_A_CLUB_SITE };
