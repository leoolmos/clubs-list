...
const GAP_MS = parseInt(process.env.SEARCH_GAP_MS || '6000', 10);
const GAP_MAX_MS = parseInt(process.env.SEARCH_GAP_MAX_MS || '15000', 10);
let lastQuery = 0;
 ...
 * edge itself instead of guessing at it once. */
let gapMs = GAP_MS;
let answeredRun = 0;
function onAnswered(){ if(++answeredRun >= 30){ answeredRun = 0; gapMs = Math.max(GAP_MS, Math.round(gapMs * 0.9)); } }
function onRefused(){ answeredRun = 0; gapMs = Math.min(GAP_MAX_MS, Math.round(gapMs * 1.5)); }
function currentGapMs(){ return gapMs; }

/* When the engine turns us away it is asked again once on the other
 * endpoint, then once more after a real pause. Turned away three times, it
 * is left alone for a while: every call inside the cooldown answers
 * "throttled" at once rather than adding to the pile, and the caller keeps
 * its place in the queue. Measured 2026-08-23: at a 2.5 s gap the engine
 * refused after 37 queries in a row; the gap is four seconds now. */
const RETRY_WAIT_MS = parseInt(process.env.SEARCH_RETRY_WAIT_MS || '60000', 10);
...
async function searchWeb(query, timeoutMs){
   ...
   * expensive: the lead is marked searched and never looked at again. So an
   * empty answer is asked again — once on each endpoint with the ordinary
   * gap, then once more after a real pause — and only then reported, with
   * `throttled` set on the (empty) array so the caller can tell a refusal
   * from a country with no clubs and keep its place in the queue. */
  if(Date.now() < cooldownUntil) return throttled([], 'cooling down');

  let html = null, res = null, tries = 0;
  for(let attempt = 0; attempt < 3 && !html; attempt++){
    // 0: html, 1: lite, 2: html again after a long pause
    const endpoint = ENDPOINTS[attempt % ENDPOINTS.length];
    const base = attempt < 2 ? gapMs : RETRY_WAIT_MS;
    const jitter = 0.6 + Math.random() * 0.8;            // 60%–140% of the gap
    const wait = Math.round(base * jitter) - (Date.now() - lastQuery);
    if(wait > 0) await sleep(wait);
    lastQuery = Date.now();
    tries++;

    res = await fetchSearch(endpoint + encodeURIComponent(query), timeoutMs || 9000);
    if(res.text && /uddg=/.test(res.text)){ html = res.text; break; }
    if(!refused(res)) return [];                          // a real "no results"
  }
  if(!html){
    ...
    return throttled([], `refused ${tries} times (last status ${res ? res.status : 0}` +
                         (lastRefusal.excerpt ? `: "${lastRefusal.excerpt.slice(0, 90)}"` : '') + ')');
  }
