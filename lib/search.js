...
const GAP_MS = parseInt(process.env.SEARCH_GAP_MS || '2500', 10);
let lastQuery = 0;

/* When the engine turns us away it is asked again once, after a real pause,
 * on the other endpoint. Turned away twice, it is left alone for a while:
 * every call inside the cooldown answers "throttled" at once rather than
 * adding to the pile, and the caller keeps its place in the queue. */
const RETRY_WAIT_MS = parseInt(process.env.SEARCH_RETRY_WAIT_MS || '45000', 10);
const COOLDOWN_MS   = parseInt(process.env.SEARCH_COOLDOWN_MS   || '180000', 10);
let cooldownUntil = 0;
...
async function searchWeb(query, timeoutMs){
  ...
  for(let attempt = 0; attempt < 4 && !html; attempt++){
    // 0: html, 1: lite, 2: html after a long pause, 3: lite after a pause
