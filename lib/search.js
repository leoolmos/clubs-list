...
/* A question the engine has never objected to, used to tell a refusal of
 * this client from a refusal of one query's wording. Ordinary words, a real
 * city, nothing that looks like harvesting. */
const CANARY = 'tennis club London contact';
...
/* Ten seconds between queries. The pace was never the problem: measured
 * 2026-08-26, twelve queries four seconds apart were all answered, while
 * the collector beside them was refused on its first — the difference was
 * the word "email" in half of its questions, not the interval. Ten seconds
 * is ordinary politeness towards a free service; the gap below climbs on
 * its own if the engine ever does object to the pace. */
const GAP_MS = parseInt(process.env.SEARCH_GAP_MS || '10000', 10);
/* The gap is the throttle, and it has to be allowed to grow past the point
 * where the engine is comfortable. Capped at twenty seconds it could not:
 * the pressure went into the resting time instead, and a day of that made
 * thirty-seven queries. Two minutes between queries is still seven hundred
 * a day, twenty times what the old ceiling managed. */
const GAP_MAX_MS = parseInt(process.env.SEARCH_GAP_MAX_MS || '120000', 10);
let lastQuery = 0;

/* The gap adapts. Every refusal stretches it by half, up to GAP_MAX_MS;
 * every twenty answered queries in a row shrink it a tenth, never below
...
let gapMs = GAP_MS;
let answeredRun = 0;
/* Set by a refusal, cleared by the first answer after it. That answer says
 * the engine is talking to us again, and the gap comes down at once rather
 * than waiting for the shrinking below to grind it down. */
let recovering = false;
/* Requests that got no answer at all — no status, a socket that never
 * opened, a timeout. Counted apart from refusals, because they are not the
 * engine's anomaly detector speaking: that one answers, with a 202 and a
 * page. See the outage path in searchWeb. */
let outagesInARow = 0;
function onAnswered(){
  outagesInARow = 0;
  /* A quarter off on the first answer after a rest. The doubling was earned
   * by a refusal, and the rest of five to fifteen minutes is what clears
   * the engine's window; from there the pace should look for the edge
   * again, not sit at two minutes a query for the rest of the day.
   * Measured 2026-09-06: the gap sat at 120 s for hours while the engine,
   * asked by hand, answered instantly — thirty queries an hour on a line
   * that was open. */
  if(recovering){ recovering = false; gapMs = Math.max(GAP_MS, Math.round(gapMs * 0.75)); }
  /* A fifth off every ten answers, not a tenth every twenty: from the
   * ceiling back to the floor is about a hundred answered queries, an hour
   * or so, instead of five hundred. */
  if(++answeredRun >= 10){ answeredRun = 0; gapMs = Math.max(GAP_MS, Math.round(gapMs * 0.8)); }
  if(answeredRun >= 5) refusalsInARow = 0;              // the engine is talking to us again
}
/* Doubling, not half again: the edge is found in two or three refusals
 * instead of six, and every refusal costs a rest to discover. */
function onRefused(){ answeredRun = 0; recovering = true; gapMs = Math.min(GAP_MAX_MS, Math.round(gapMs * 2)); }
...
/* Fifteen minutes, not an hour. The hour was a trap: resting that long
 * leaves time for exactly one attempt before the next round, so the five
 * answers in a row that clear the counter can never happen, one unlucky
 * attempt re-arms the full hour, and the engine is idle all day. Measured
 * 2026-08-26: 37 real attempts in 24 hours, and DuckDuckGo answering three
 * queries in a row the moment it was asked by hand. */
const COOLDOWN_MAX_MS = parseInt(process.env.SEARCH_COOLDOWN_MAX_MS || '900000', 10);
/* A rest for a line that is down, as against one closed to us: a minute,
 * doubling to five. Long enough not to hammer a network that is having a
 * moment, short enough that a blip costs a minute and not an afternoon. */
const OUTAGE_REST_MS     = parseInt(process.env.SEARCH_OUTAGE_REST_MS     || '60000',  10);
const OUTAGE_REST_MAX_MS = parseInt(process.env.SEARCH_OUTAGE_REST_MAX_MS || '300000', 10);
/* Twelve seconds at least for the engine's answer. It answers in under a
 * second when it answers at all, but a slow evening on this side made nine
 * seconds a timeout, and a timeout used to be read as a refusal. */
...
async function searchWeb(query, timeoutMs){
  ...
  /* Irregular, but never faster than the gap. Sixty to a hundred and forty
   * per cent was the flaw underneath all of this: a gap stretched to
   * thirty-four seconds by three refusals was thrown back to twenty on the
   * next query about half the time, which earned the next refusal, and the
   * adaptation those refusals paid for was undone as fast as it was made. */
  const jitter = 1 + Math.random() * 0.4;                // 100%–140% of the gap
  const wait = Math.round(gapMs * jitter) - (Date.now() - lastQuery);
  if(wait > 0) await sleep(wait);
  lastQuery = Date.now();
  let tries = 1;
  const ms = Math.max(timeoutMs || 0, SEARCH_TIMEOUT_MS);

  let html = null;
  let res = await fetchSearch(endpoint + encodeURIComponent(query), ms);

  /* No answer at all — no status, nothing on the wire — is not the engine
   * declining. Its refusal is a page, served with a 202, and every request
   * made while that page is up counts against us; a socket that never
   * opened counted for nothing. So this one case is asked once more, on
   * the other endpoint, before anything is concluded — and if that gets
   * nothing either, the line is down rather than closed to us: a short
   * rest, no doubling of the gap, and no canary, which would only time out
   * as well.
   *
   * This is the difference between an evening and a day. Fifteen timeouts
   * in a row on 2026-09-06 were read as fifteen refusals: the gap went to
   * its two-minute ceiling and the rest to its quarter-hour, and both
   * stayed there for hours while the engine was answering anyone who
   * asked. */
  if(!res.status){
    await sleep(3000);
    tries = 2;
    res = await fetchSearch(ENDPOINTS[queryNo % ENDPOINTS.length] + encodeURIComponent(query), ms);
    if(!res.status){
      outagesInARow++;
      const restMs = Math.min(OUTAGE_REST_MAX_MS, OUTAGE_REST_MS * Math.pow(2, outagesInARow - 1));
      cooldownUntil = Date.now() + restMs;
      lastRefusal = {at: Date.now(), status: 0, tries, query, excerpt: '', inARow: outagesInARow,
                     restMs, cooldownMin: Math.round(restMs / 60000), outage: true};
      return throttled([], `no answer from the engine at all (${outagesInARow} in a row, both endpoints), ` +
                           `resting ${Math.round(restMs / 1000)} s`);
    }
  }
  ...
  if(!html){
    /* Is it us, or is it this question? The engine refuses individual
     * queries on their wording — any query with the word "email" in it gets
     * the challenge page whatever the pace — and reading that as rate
     * limiting cost three days: the phase rested, the rest doubled, and the
     * next round asked the same poisoned query first.
     *
     * So a refusal is checked against a canary the engine has no reason to
     * object to. Canary answers: the question was the problem, and it is
     * reported as poisoned so the caller can drop it and carry on. Canary
     * refused too: it really is us, and the rest below applies. */
    const canaryWait = Math.round(gapMs * (1 + Math.random() * 0.4)) - (Date.now() - lastQuery);
    if(canaryWait > 0) await sleep(canaryWait);
    lastQuery = Date.now();
    const canary = await fetchSearch(ENDPOINTS[0] + encodeURIComponent(CANARY), timeoutMs || 9000);
    if(canary.text && /uddg=/.test(canary.text)){
      onAnswered();
      const out = [];
      out.throttled = true;
      out.poisoned = true;
      out.why = 'the engine refuses this wording (the canary was answered)';
      return out;
    }

    // A refusal long after the last one is a fresh burst, not a continuation
    // of it. Without this the counter only ever climbed — answers are what
    // used to clear it, and at the top of the scale there is barely time to
    // ask — so the rest ratcheted to its maximum and stayed there.
    const sinceLast = lastRefusal ? Date.now() - lastRefusal.at : Infinity;
    if(sinceLast > 2 * (lastRefusal && lastRefusal.restMs || COOLDOWN_MS)) refusalsInARow = 0;
    refusalsInARow++;
    const restMs = Math.min(COOLDOWN_MAX_MS, COOLDOWN_MS * Math.pow(2, refusalsInARow - 1));
    cooldownUntil = Date.now() + restMs;
    onRefused();
    ...
    lastRefusal = {at: Date.now(), status: res ? res.status : 0, tries, query,
                   excerpt: body.slice(0, 160), inARow: refusalsInARow, restMs,
                   cooldownMin: Math.round(restMs / 60000)};
  ...
  if(PAGES > 1 && (html.match(/uddg=/g) || []).length >= 10){
    const wait = Math.round(gapMs * (1 + Math.random() * 0.4)) - (Date.now() - lastQuery);
    if(wait > 0) await sleep(wait);
    lastQuery = Date.now();
...
/* How long the engine has asked to be left alone, in milliseconds. The
 * caller uses it to wait the rest out inside the round instead of ending
 * the round on it: a round that stops at the first refusal, when the crawl
 * has nothing to do either, is over in a tenth of a second and the whole
 * cooldown is spent resting on top of the rest between rounds. */
function searchCooldownMs(){ return Math.max(0, cooldownUntil - Date.now()); }
function lastSearchRefusal(){ return lastRefusal; }

module.exports = { searchWeb, findClubSite, searchCoolingDown, searchCooldownMs, lastSearchRefusal,
                   currentGapMs, ENDPOINT, ENDPOINTS, NOT_A_CLUB_SITE };
