...
/* The country's name as its own clubs write it. A search in Spanish for
 * "club de tenis Sevilla Spain contacto" mixes languages and ranks worse than
 * "club de tenis Sevilla España contacto"; a Brazilian club's contact page
 * says Brasil and never Brazil, so the proof-of-place test has to know the
 * local spelling too. English-speaking countries keep their English name,
 * shortened to what people actually type for the two big ones. */
const LOCAL_NAMES = {
  DO:'República Dominicana', GQ:'Guinea Ecuatorial', MX:'México', PA:'Panamá', PE:'Perú',
  ES:'España',
  BR:'Brasil', CV:'Cabo Verde', TL:'Timor-Leste', GW:'Guiné-Bissau', MZ:'Moçambique',
  ST:'São Tomé e Príncipe',
  GB:'UK', US:'USA', AE:'UAE'
};
function localNameOf(cc){ return LOCAL_NAMES[String(cc).toUpperCase()] || nameOf(cc); }

/* Every way a page names the country, for the proof-of-place test: the
 * English name, the local name, and the forms the big English-speaking
 * countries go by on their own sites. */
const PLACE_ALIASES = {
  ...
  ST: ['São Tomé and Príncipe','Sao Tome and Principe','São Tomé e Príncipe','Sao Tome e Principe','São Tomé'],
  GW: ['Guinea-Bissau','Guiné-Bissau','Guine-Bissau'],
  MZ: ['Mozambique','Moçambique','Mocambique'],
  BR: ['Brazil','Brasil'], MX: ['Mexico','México'], PA: ['Panama','Panamá'], PE: ['Peru','Perú'], ES: ['Spain','España']
};
function placeNames(cc){
  cc = String(cc).toUpperCase();
  const set = new Set([nameOf(cc), localNameOf(cc)].concat(PLACE_ALIASES[cc] || []).filter(Boolean));
...
module.exports = { COUNTRIES, CODES, PRIORITY, TLD_TO_CC, LOCAL_NAMES, PLACE_ALIASES,
                   isWanted, nameOf, langOf, localNameOf, placeNames, ccFromHost };
