...
function toRecord(row, cc){
   ...
   * there is no per-place URL to link to. The release the row was read from
   * is the honest equivalent — it is what someone checking the record would
   * need to go and find it again. */
  const rec = {
    name, cc, country: nameOf(cc), lang: langOf(cc), sports,
    website, email, contact: '',
    src: 'overture',
    srcPage: 'https://docs.overturemaps.org/guides/places/',
    release: row.release || '',
    crawled: !!email,
    attempts: 0
  };
